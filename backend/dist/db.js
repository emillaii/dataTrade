...
export async function insertDataset(meta, client) {
    const c = client ?? (await pool.connect());
    try {
...
export async function insertCandles(datasetId, candles, client) {
    if (candles.length === 0)
        return;
    const c = client ?? (await pool.connect());
    try {
        ...
        const valueStrings = candles.map((bar, idx) => {
            const base = idx * 9;
            values.push(datasetId, bar.symbol, bar.timeframe, bar.timestamp, bar.open, bar.high, bar.low, bar.close, bar.volume);
...
export async function listDatasets(filters = {}) {
    const { symbol, timeframe, from, to, limit = 50, offset = 0 } = filters;
    const params = [];
    const where = [];
    if (symbol) {
        params.push(symbol);
...
export async function updateDatasetTimezone(id, timezone, client) {
    const c = client ?? (await pool.connect());
    try {
        const res = await c.query(`
      UPDATE datasets
