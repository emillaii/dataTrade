...
const parseTimestamp = (primary: any, secondary?: any) => {
  if (primary === undefined || primary === null) return NaN;
...
app.post("/api/import", upload.single("file"), async (req, res) => {
  ...
  const symbol = (req.body.symbol as string) || derived.symbol || "UNKNOWN";
  const timeframe = (req.body.timeframe as string) || derived.timeframe || "M15";
  const timezone = (req.body.timezone as string) || derived.timezone || "UTC";
  const tzOffsetMs = parseTimezoneOffsetMinutes(timezone) * 60 * 1000;

  const filePath = req.file.path;

  const candles: BarEvent[] = [];
  ...
  try {
    ...
    for await (const record of stream) {
      ...
      let v: any;
      let rowSymbol: any;
      let rowTimeframe: any;

      if (Array.isArray(row)) {
        // Try [date, time, open, high, low, close, volume]
        if (row.length >= 6) {
          ts = parseTimestamp(row[0], row[1]);
          if (Number.isNaN(ts)) ts = parseTimestamp(row[0]); // maybe first col is timestamp
        ...
        for (const key of Object.keys(r)) {
          lower[key.toLowerCase()] = (r as any)[key];
        }
      ...
      if (!timestampLocal || Number.isNaN(timestampLocal)) continue;
    ...
    try {
      ...
      const dataset = await insertDataset(
        { symbol, timeframe, timezone, sourceFile, rows, startTime, endTime },
        client
      );
...
function deriveMetaFromFilename(name: string) {
